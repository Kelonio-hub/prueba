...
const MODEL = 'gemini-1.5-flash-latest';  // Modelo rápido y gratuito
...
async function enviarMensaje() {
    ...
    // Preparar el prompt para Gemini (system + user)
    ...
    **Instrucciones**:
    ...
    // Preparar el cuerpo de la solicitud para Gemini
    const requestBody = {
        contents: [
            {
                parts: [
    ...
    // Log para depuración
    console.log('Cuerpo de la solicitud a Gemini:', JSON.stringify(requestBody, null, 2));
    ...
    try {
        ...
        const respuesta = data.candidates && data.candidates[0] && data.candidates[0].content && data.candidates[0].content.parts && data.candidates[0].content.parts[0] && data.candidates[0].content.parts[0].text ? 
            data.candidates[0].content.parts[0].text.trim() : 'Lo siento, no pude procesar tu solicitud en este momento.';

        // Agregar respuesta del bot al chat
        agregarMensaje(respuesta);
    } catch (error) {
        console.error('Error al conectar con la API de Gemini:', error);
        showToast(`Error: ${error.message}. Intenta de nuevo más tarde.`, 'error');
