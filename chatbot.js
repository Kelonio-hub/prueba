 ...
 * Lee Catálogo.xml (antes Ejemplares.xml) con tags compatibles con index.html.
 * Responde con Groq desde el inicio, con filtros y sugerencias.
 * Fallback local si falla, sin bloquear splash.
 */
...
/**
 * Carga Catálogo.xml con tags compatibles con index.html.
 * @returns {Promise<Array>} Lista de libros.
 */
async function cargarCatalogo() {
    if (catalogo) {
        console.log('Usando catálogo existente:', catalogo.length, 'libros');
        return catalogo;
    }
    ...
    try {
        ...
        const response = await fetch('https://kelonio-hub.github.io/prueba/Catálogo.xml', { signal: controller.signal });
        ...
        console.log('Catálogo cargado:', catalogo.length, 'libros');
        return catalogo;
    } catch (error) {
        console.error('Error cargando Catálogo.xml:', error);
        catalogo = [];
        return [];
    }
}
