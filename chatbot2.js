    ...
    try {
        ...
        if (estadoChat === 'inicio' && mensaje) {
            if (mensaje.includes('recomienda') || mensaje.includes('sugiere')) {
                ...
                return;
            } else if (mensaje.includes('puntaje') || mensaje.includes('puntuación')) {
        ...
        if (estadoChat === 'inicio') {
            const opciones = [
                { texto: 'Recomendaciones', accion: 'recomendaciones' },
                ...
                { texto: 'Ver puntaje total', accion: 'ver_puntaje' }
            ];
            setTimeout(() => {
            ...
            if (esCorrecta) {
                ...
                if (aciertosConsecutivos >= 3) {
                    ...
                    respuesta = `¡Increíble, tres aciertos seguidos! 🎉 +30 puntos extra. Puntuación: ${puntuacion}. ${datoCurioso}`;
                } else {
                    respuesta = `¡Genial, +5 puntos! 📖 Puntuación: ${puntuacion}. ${datoCurioso}`;
                }
            } else {
                aciertosConsecutivos = 0;
                respuesta = `¡Uy, casi! La respuesta correcta era ${tipo === 'autor' ? libro.autor : tipo === 'categoria' ? libro.categoria : libro.fechaEdicion}. ${datoCurioso} Puntuación: ${puntuacion}.`;
            }
        ...
        if (opcion === 'recomendaciones') {
            ...
            setTimeout(() => {
                agregarMensaje('¿Qué tipo de recomendaciones quieres? 📚', 'bot', [
                    { texto: 'Generales', accion: 'recomendaciones_generales' },
                    { texto: 'Personalizadas', accion: 'recomendaciones_personalizadas' }
                ]);
                input.disabled = false;
                input.focus();
            }, 1000);
            ...
            estadoChat = 'trivial';
            const nivel = puntuacion < 20 ? 'fácil' : puntuacion < 50 ? 'medio' : 'difícil';
            ...
            const respuesta = puntuacion > 0
                ? `¡Eres un genio literario! 🌟 Tu puntaje total es: ${puntuacion}.`
                : '¡Aún no tienes puntos! 😊 Juega al trivial para sumar algunos. ❓';
            setTimeout(() => {
                agregarMensaje(respuesta, 'bot');
                estadoChat = 'inicio';
                agregarMensaje('¿Qué más quieres hacer?', 'bot', [
            ...
            setTimeout(() => {
                agregarMensaje(`¡Fin del juego! Tu puntaje total es: ${puntuacion}. ¡Eres un genio literario! 📖`, 'bot');
                puntuacion = 0;
                aciertosConsecutivos = 0;
                localStorage.setItem('trivialPuntuacion', puntuacion);
                localStorage.setItem('aciertosConsecutivos', aciertosConsecutivos);
            ...
            if (esCorrecta) {
                ...
                if (aciertosConsecutivos >= 3) {
                    puntuacion += 30;
                    aciertosConsecutivos = 0;
                    respuesta = `¡Increíble, tres aciertos seguidos! 🎉 +30 puntos extra. La categoría era *${trivialPreguntaActual.libro.categoria}*. Puntuación: ${puntuacion}.`;
                } else {
                    respuesta = `¡Correcto! La categoría era *${trivialPreguntaActual.libro.categoria}*. +5 puntos. Puntuación: ${puntuacion}.`;
                }
            } else {
                aciertosConsecutivos = 0;
                respuesta = `¡Ups! La categoría correcta era *${trivialPreguntaActual.libro.categoria}*. Puntuación: ${puntuacion}.`;
            }
            localStorage.setItem('trivialPuntuacion', puntuacion);
            localStorage.setItem('aciertosConsecutivos', aciertosConsecutivos);
