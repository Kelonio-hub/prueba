async function initApp() {
    ...
    try {
        loadTheme();
        await cargarDatos(); // Esto podría fallar, pero lo capturamos
        const emailSesionGuardada = sessionStorage.getItem('emailSesion');
        if (emailSesionGuardada) {
            console.log('Sesión activa encontrada: ', emailSesionGuardada);
        ...
        showToast('Error al inicializar la aplicación. Mostrando login.', 'error');
        document.getElementById('loginSection').style.display = 'block'; // Mostrar login en caso de error
        document.getElementById('app').style.display = 'none';
    }

    // Asegura que el splash se oculte siempre después de 3 segundos
