...
async function cargarCatalogo() {
    try {
        ...
        const responseLibros = await fetch('Ejemplares.xml'); // Ajusta el nombre del archivo
        if (!responseLibros.ok) {
            throw new Error(`Error HTTP: ${responseLibros.status} - No se pudo cargar Ejemplares.xml`);
        }
...
async function enviarMensaje() {
    ...
    if (mensajeLower.includes('buscar') || mensajeLower.includes('libro')) {
        respuesta = 'Usa el buscador de arriba para encontrar libros por título (ej. "Cuando Dios aprieta"), autor (ej. "Guillermo Fesser"), ISBN o categoría (ej. "Narrativa") en la Biblioteca Carpe Diem, Fuenlabrada. ¿Quieres un ejemplo?';
    } else if (mensajeLower.includes('disponible') || mensajeLower.includes('prestado')) {
        respuesta = 'Los libros con estado "Sí" están disponibles para préstamo en la Biblioteca Carpe Diem, Fuenlabrada. Si dice "No", están prestados. Prueba buscar por título o autor para verificar un libro específico.';
        ...
        respuesta = 'Puedes buscar libros por autor, como "Guillermo Fesser", en la Biblioteca Carpe Diem, Fuenlabrada. Dime un autor o género para ayudarte a encontrar algo.';
    } else if (mensajeLower.includes('biblioteca')) {
        respuesta = '¡Bienvenido a la Biblioteca Escolar Carpe Diem, Fuenlabrada! Usa el buscador para encontrar libros por título, autor, ISBN o categoría. Si necesitas ayuda con préstamos o contactar al bibliotecario, dime más.';
    } else {
        respuesta = `Hola, soy el asistente de la Biblioteca Carpe Diem, Fuenlabrada. Tu pregunta: "${mensaje}". Prueba con palabras como "buscar", "recomendación", "autor" o "disponible" para obtener ayuda con el catálogo. ¿Qué necesitas?`;
    }
