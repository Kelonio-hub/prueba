// Cargar datos del catálogo let catalogo = [];

async function cargarCatalogo() { try { const response = await fetch('Ejemplares.xml'); // Ajusta el nombre del archivo XML if (!response.ok) { throw new Error(Error HTTP: ${response.status} - No se pudo cargar Ejemplares.xml); } const xmlText = await response.text(); const parser = new DOMParser(); const xmlDoc = parser.parseFromString(xmlText, 'text/xml');
...
// Búsqueda de ejemplares function buscarLibros() { if (!catalogo.length) { document.getElementById('resultados').innerHTML = '

El catálogo aún no se ha cargado. Intenta de nuevo en unos segundos.

'; return; } const query = document.getElementById('buscarInput').value.toLowerCase(); const resultados = catalogo.filter(ejemplar => ejemplar.signatura.toLowerCase().includes(query) // ya no se busca por ISBN ); mostrarResultados(resultados); }
function mostrarResultados(ejemplares) { const div = document.getElementById('resultados'); if (ejemplares.length === 0) { div.innerHTML = '

No se encontraron ejemplares.

'; return; } div.innerHTML = ejemplares.map(ejemplar =>  <div class="libro"> <h3 class="signatura">${ejemplar.signatura || 'Sin signatura'}</h3> <p><strong>Disponible:</strong> <span class="${ejemplar.estado ? 'estado-disponible' : 'estado-no-disponible'}">${ejemplar.estado ? 'Sí' : 'No'}</span></p> </div> ).join(''); }
// Chatbot SIMULADO (sin API real) async function enviarMensaje() { const input = document.getElementById('chatInput'); const mensaje = input.value.trim(); if (!mensaje) return;
...
function agregarMensaje(texto, tipo) { const div = document.getElementById('chatMensajes'); const p = document.createElement('p'); p.className = mensaje ${tipo}; p.textContent = texto; div.appendChild(p); div.scrollTop = div.scrollHeight; }

// Inicializar al cargar la página cargarCatalogo();